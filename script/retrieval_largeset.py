...
def main():
    print("="*50)
    print("🧠 MEG Brain Decoding: Large Set Retrieval (Top-5)")
    print("="*50)
    
    # 1. Ensure features are extracted!
    _, path_large = ensure_features(device=DEVICE)
    
    # 2. Load the ground truth large-set features
    print(f"\n📥 Loading Large Set ground-truth features...")
    with h5py.File(path_large, 'r') as f:
        gt_autokl = f['autokl'][:]
    ...
    num_large = len(gt_cats)
    print(f"   Identified {num_large} unique images in Large Set.")
    ...
    for sub_idx, sub in enumerate(subjects):
        ...
        # We index the gt_images so we know exactly the 2400-long vector alignment
        pred_accums = {
            ...
            for i in tqdm(range(0, N, batch_size), desc=f"Predicting {sub}"):
                ...
                for b in range(len(batch_meg)):
                    ...
                    if img_pair in gt_images:
                        # Unrepeated, so we only append once per pair, but let's use list simply.
        ...
        print(f"   Gathering predictions & calculating Top-5 Acc over {num_large} candidates...")
        ...
        for idx_gt, img_pair in enumerate(gt_images):
            ...
            if len(pred_accums['autokl'][img_pair]) > 0:
                # Should be only 1, but we use .mean() to collapse the list safely
        ...
        if len(final_preds['autokl']) == 0:
            print(f"   No large set images found for {sub}.")
            continue
