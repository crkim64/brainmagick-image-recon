...
class PositionGetter:
    ...
    def get_recording_layout(self, recording=None) -> torch.Tensor:
        # Modified to use static sensor positions from P1
        index = 0 # Default index
        if index in self._cache:
            return self._cache[index]
        else:
            try:
                # Load the provided P1 sensor positions
                positions_np = np.load('data/sensor_positions/sensor_positions_P1.npy')
                positions = torch.from_numpy(positions_np).float()
                
                # Check shape, P1 has 271, but model expects 272 (one missing/padded?)
                # The user provided P1.npy has 271 sensors. 
                # BrainModule default in_channels is 272.
                # If we return 271, it might mismatch if the input tensor is (B, 272, T).
                # However, PositionGetter is used to get positions for the CHANNELS.
                # If the input data has 272 channels, we need 272 positions.
                # Let's check if we should pad or if the input will be 271.
                # The user said "sensor_positions_P1.npy ... use it".
                # For now, let's return what is in the file.
                
                self._cache[index] = positions
                return positions
            except Exception as e:
                logger.error(f"Failed to load sensor positions: {e}")
